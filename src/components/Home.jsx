...
>
        ...
        <h1 className="text-4xl font-bold drop-shadow-md animate-slideUp">AgriFlow – AI-Powered Smart Irrigation</h1>
      ...
      <section className="py-12 bg-white animate-fadeInUp">
        <h2 className="text-2xl font-bold text-center mb-6">Why Choose AgriFlow?</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 px-6 max-w-5xl mx-auto">
          {["💧 AI Smart Irrigation", "⛅ Live Weather Data", "📢 Smart Alerts", "🌾 Crop Health Monitoring", "📊 Market Price Tracker"].map((feature, index) => (
            <div 
              key={index} 
      ...
      <section className="py-12 bg-green-100 animate-fadeInUp">
        <h2 className="text-2xl font-bold text-center mb-6">Live Weather & Soil Stats</h2>
        <div className="max-w-3xl mx-auto flex flex-col md:flex-row justify-center gap-6 text-lg">
          {[
            { icon: "🌡", label: "Temperature", value: "28°C", color: "text-blue-600" },
            { icon: "💦", label: "Soil Moisture", value: "45%", color: "text-green-600" },
            { icon: "⏳", label: "Next Irrigation", value: "In 3 Days", color: "text-red-600" }
          ].map((stat, index) => (
            <div 
              key={index} 
            ...
            >
              {stat.icon} {stat.label}: <span className={`font-bold ${stat.color}`}>{stat.value}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Testimonials */}
      <section className="py-12 bg-white animate-fadeInUp">
        <h2 className="text-2xl font-bold text-center mb-6">Farmers' Testimonials</h2>
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 px-6">
          {[
            { text: "AgriFlow saved me 30% of water while increasing my crop yield!", author: "Ramesh, India" },
            { text: "Best smart irrigation tool! Easy to use and highly accurate.", author: "Sarah, USA" }
